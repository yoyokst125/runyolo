...
from utils.custom_optimizers import RK4Optimizer
...
def train(hyp, opt, device, callbacks):
    ...
    amp = False  # <--- 強制禁用 AMP 進行初步測試
    LOGGER.info(f"Autocast (AMP) forced to: {amp} for RK4 testing")
    ...
    accumulate = 1  # <--- 強制 accumulate 為 1 進行初步測試
    LOGGER.info(f"Gradient Accumulation forced to: {accumulate} for RK4 testing")
    hyp["weight_decay"] *= batch_size * accumulate / nbs  # scale weight_decay
    optimizer = smart_optimizer(model, opt.optimizer, hyp["lr0"], hyp["momentum"], hyp["weight_decay"], hyp=hyp)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # Forward
            # --- 修改從這裡開始 (在 for i, (imgs, targets, paths, _) in pbar: 循環內部) ---

            # Forward (計算一次初始損失，主要用於日誌和可能的返回)
            # 由於 amp=False，torch.cuda.amp.autocast(amp) 不會產生影響
            with torch.cuda.amp.autocast(enabled=amp): # amp 已被我們設為 False
                pred = model(imgs)
                loss_initial, loss_items_initial = compute_loss(pred, targets.to(device)) 
            
            if RANK != -1: # DDP loss scaling
                loss_initial *= WORLD_SIZE
            # if opt.quad: loss_initial *= 4.0 # 如果您的配置使用此選項

            # ---- 判斷優化器類型，執行不同的梯度計算和更新步驟 ----
            if isinstance(optimizer, RK4Optimizer):
                # 由於 accumulate 強制為 1，這個 if 條件每個 batch 都會執行
                if ni - last_opt_step >= accumulate: 
                    def closure():
                        # RK4Optimizer 內部的 get_grads_after_closure_and_clear_previous 
                        # 會在調用此 closure 前清除模型參數的梯度。
                        # 因此，此 closure 只需執行：前向傳播 -> 計算損失 -> 反向傳播
                        
                        # with torch.cuda.amp.autocast(enabled=amp): # amp 已禁用
                        _pred = model(imgs) # 重新執行前向（因為參數可能在RK4內部被修改）
                        _loss, _ = compute_loss(_pred, targets.to(device)) # _loss_items 通常不需要在closure內部更新
                        
                        if RANK != -1:
                            _loss *= WORLD_SIZE
                        # if opt.quad: _loss *= 4.0
                        
                        _loss.backward() # 計算當前參數點的梯度
                        return _loss

                    loss_returned_by_optimizer = optimizer.step(closure) # 執行 RK4 優化步驟
                    loss = loss_returned_by_optimizer # 更新 loss 變數，以便後續日誌使用
                    loss_items = loss_items_initial # 保持第一次計算的 loss_items 用於日誌細分

                    # optimizer.zero_grad() 由 RK4Optimizer 的 closure 調用前的邏輯處理，
                    # 或者在 RK4 優化器內部管理。對於外部的優化步驟完成後，
                    # 如果 accumulate > 1 (雖然我們這裡強制為1)，則標準做法是在此清除。
                    # 對於 accumulate = 1，並且 RK4 內部已處理，這裡可能不需要。
                    # 但為了安全起見，或如果 RK4 的實現沒有在 step 結束時清除，可以保留。
                    # 我們的 RK4Optimizer 內部會在每次 get_grads_after_closure_and_clear_previous 時清除。
                    # 所以這裡的 optimizer.zero_grad() 是針對完成一個優化步驟後。
                    if accumulate == 1 : # 如果是每個 batch 都 step
                         optimizer.zero_grad(set_to_none=True) # 在RK4 step完成後立即清零是安全的

                    if ema:
                        ema.update(model)
                    last_opt_step = ni
                # else: # accumulate > 1 的情況，對於RK4，我們建議 accumulate=1，所以此分支不應進入
                #     loss_initial.backward() # 若要支持 accumulate > 1 for RK4, 這裡需要更複雜的設計
                #     pass

            else: # 原有的 SGD, Adam, AdamW 等優化器邏輯 (amp=False, accumulate=1 的情況)
                loss_initial.backward() # 計算梯度 (loss_initial 是上面計算的)
                
                if ni - last_opt_step >= accumulate: # 因為 accumulate=1，這個條件會滿足
                    # 梯度裁剪 (可以保留，max_norm 來自超參數)
                    # 注意: model.hyp 可能需要在 train() 函數更早的地方被賦值
                    # 或者直接使用固定的 max_norm=10.0
                    clip_norm = hyp.get('grad_clip_norm', 10.0) # 從超參數獲取，預設10.0
                    if clip_norm > 0:
            ...
            # Log (日誌部分使用更新後的 loss 和 loss_items)
            if RANK in {-1, 0}:
                # mloss 的計算依賴於 loss_items，確保 loss_items 被正確賦值
                # 如果是 RK4，我們使用 loss_items_initial
                # 如果是其他優化器，compute_loss 在 if/else 塊之前只調用了一次，loss_items_initial 就是最新的
                mloss = (mloss * i + loss_items_initial) / (i + 1)  # update mean losses
                ...
                if callbacks.stop_training:
                    return # 直接從 train 函數返回
            # end batch --- (這是註釋，不是程式碼)
# --- 修改到這裡結束 ---

            # --- 判斷優化器類型，執行不同的梯度計算和更新步驟 ---
            # 確保 loss 和 loss_items 變數在 if/else 作用域之外也能被 Log 部分訪問
            current_batch_loss = loss_initial # 先用初始計算的 loss (來自之前的 Forward 部分)
            current_batch_loss_items = loss_items_initial # 先用初始計算的 loss_items

            if isinstance(optimizer, RK4Optimizer):
                # 因為 accumulate 強制為 1，這個 if 條件每個 batch 都會執行
                if ni - last_opt_step >= accumulate:
                    def closure():
                        # RK4Optimizer 內部的 get_grads_after_closure_and_clear_previous 會清除梯度
                        # closure 只需執行：前向傳播 -> 計算損失 -> 反向傳播
                        ...
                        if RANK != -1: # DDP loss scaling
                            _loss *= WORLD_SIZE
                        # if opt.quad: _loss *= 4.0 # 如果您的配置使用此選項
                        
                        _loss.backward() # 計算當前參數點的梯度
                        return _loss

                    loss_from_rk4_step = optimizer.step(closure) # 執行 RK4 優化步驟
                    current_batch_loss = loss_from_rk4_step # 更新 loss 變數，以便後續日誌使用
                    # current_batch_loss_items 保持使用 loss_items_initial，因為 RK4 的 closure 不方便返回細分的 loss_items

                    # optimizer.zero_grad(set_to_none=True) # 在 RK4Optimizer 內部已處理每個子步驟的梯度清除
                    # 在一個完整的 RK4 step 之後，也應該清除，確保下一個 batch 乾淨
                    if accumulate == 1: # 如果是每個 batch 都 step
                    ...
                    if ema:
                        ema.update(model)
                    last_opt_step = ni
                # else: # accumulate > 1 的情況 (我們已強制 accumulate=1，此分支在初始測試中不應進入)
                #     # 如果要支持 RK4 與 accumulate > 1，這裡需要非常複雜的設計來處理梯度的 "有效" 累積
                #     # 對於初步測試，直接 backward 可能會導致梯度重複累加，除非 RK4 step 之前有 zero_grad
                #     loss_initial.backward() # 這樣梯度會累加到 model.parameters().grad
                #     pass

            else: # 原有的 SGD, Adam, AdamW 等優化器邏輯 (amp=False, accumulate=1 的情況)
                loss_initial.backward() # 計算梯度 (loss_initial 是之前 Forward 部分計算的)
                
                if ni - last_opt_step >= accumulate: # 因為 accumulate=1，這個條件會滿足
                    # scaler.unscale_(optimizer) # amp=False，不需要
                    
                    # 梯度裁剪 (可以保留，max_norm 來自超參數)
                    # 確保 hyp 字典在這裡可用，或者直接使用固定值
                    # 在 train() 函數的頂部，hyp 應該已經從 opt.hyp 或文件加載
                    clip_norm = hyp.get('grad_clip_norm', 10.0) 
                    if clip_norm > 0:
                        torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=clip_norm)
                    
                    optimizer.step() # 更新權重
                    
                    # scaler.update() # amp=False，不需要
                    ...
                    if ema:
                        ema.update(model)
                    last_opt_step = ni
                # current_batch_loss 和 current_batch_loss_items 已經是 loss_initial 和 loss_items_initial

            # Log (日誌部分使用 current_batch_loss 和 current_batch_loss_items)
            if RANK in {-1, 0}:
                # 確保 mloss 的計算使用當前 batch 正確的 loss_items
                mloss = (mloss * i + current_batch_loss_items) / (i + 1)  # update mean losses
                mem = f"{torch.cuda.memory_reserved() / 1E9 if torch.cuda.is_available() else 0:.3g}G"  # (GB)
                pbar.set_description(
                    ("%11s" * 2 + "%11.4g" * 5)
                    % (f"{epoch}/{epochs - 1}", mem, *mloss, targets.shape[0], imgs.shape[-1])
                )
                # callbacks.run 的 loss 參數也應是當前 batch 的總 loss
                callbacks.run("on_train_batch_end", model, ni, imgs, targets, paths, list(mloss)) # 假設 callbacks 可以處理額外的 loss 參數
                if callbacks.stop_training:
                    return # 直接從 train 函數返回
            # end batch ------------------------------------------------------------------------------------------------
...
def parse_opt(known=False):
    ...
    parser.add_argument("--optimizer", type=str, choices=["SGD", "Adam", "AdamW", "RK4"], default="SGD", help="optimizer")
