...
def smart_optimizer(model, name="SGD", lr=0.001, momentum=0.9, decay=1e-5, hyp=None): # 'decay' 是 weight_decay
    g = [], [], []  # optimizer parameter groups: 0-weights with decay, 1-weights no decay, 2-biases
    bn = tuple(v for k, v in nn.__dict__.items() if "Norm" in k)  # normalization layers, i.e. BatchNorm2d()
    for v in model.modules():
        for p_name, p in v.named_parameters(recurse=0):
            if not p.requires_grad: # 確保只處理需要梯度的參數
                continue
            if p_name == "bias":  # bias (no decay)
                g[2].append(p)
            elif p_name == "weight" and isinstance(v, bn):  # weight (bn, no decay)
                g[1].append(p)
            else:
                g[0].append(p)  # weight (with decay)

    if name == "Adam":
        optimizer = optim.Adam(g[2], lr=lr, betas=(momentum, 0.999)) # biases
    elif name == "AdamW":
        optimizer = optim.AdamW(g[2], lr=lr, betas=(momentum, 0.999), weight_decay=0.0) # biases, wd is handled by add_param_group
    elif name == "RMSProp":
        optimizer = optim.RMSprop(g[2], lr=lr, momentum=momentum) # biases
    elif name == "SGD":
        optimizer = optim.SGD(g[2], lr=lr, momentum=momentum, nesterov=True) # biases
    elif name == "RK4": # <-------------------------- 新增您的優化器分支
        # 現在可以直接使用傳入的 hyp 字典
        if hyp is None: # 添加一個保護，如果 hyp 沒有傳入
            hyp = {} # 或者拋出錯誤，或者使用預設值
            LOGGER.warning("Hyperparameters (hyp) not passed to smart_optimizer for RK4, using defaults for custom hyps.")

        custom_decay_val = hyp.get('weight_decay_rk4_custom', decay) # <--- 修改這裡
        # eps_custom_val = hyp.get('eps_rk4_custom', 1e-8) # <--- 示例：如果您的 RK4 有其他超參

        optimizer = RK4Optimizer([
            {'params': g[0], 'lr': lr, 'weight_decay_rk4': custom_decay_val},
            {'params': g[1], 'lr': lr, 'weight_decay_rk4': 0.0},
            {'params': g[2], 'lr': lr, 'weight_decay_rk4': 0.0}
        ], lr=lr)
    else:
        raise NotImplementedError(f"Optimizer {name} not implemented.")

    # ... (後續的 add_param_group 和 LOGGER.info 邏輯，根據 name != 'RK4' 等條件調整) ...
    if name not in ['RK4']:
    ...
    if name == 'RK4':
        ...
        for i, group in enumerate(optimizer.param_groups):
            ...
            group_wd_rk4 = group.get('weight_decay_rk4', 0.0)
            pg_details.append(f"group{i}({len(group['params'])} params, lr={group_lr:.2e}, wd_rk4={group_wd_rk4:.1e})")
        LOGGER.info(f"{colorstr('optimizer:')} {type(optimizer).__name__} with parameter groups: {'; '.join(pg_details)}")
    else:
        # 確保索引安全
        bias_params_len = len(optimizer.param_groups[0]['params']) if len(optimizer.param_groups) > 0 else 0
        decay_weights_len = len(optimizer.param_groups[1]['params']) if len(optimizer.param_groups) > 1 else 0
        decay_val_log = optimizer.param_groups[1]['weight_decay'] if len(optimizer.param_groups) > 1 else decay
        bn_weights_len = len(optimizer.param_groups[2]['params']) if len(optimizer.param_groups) > 2 else 0

        LOGGER.info(
            f"{colorstr('optimizer:')} {type(optimizer).__name__}(lr={lr}, momentum={momentum}) with parameter groups "
            f"{bias_params_len} biases, "
            f"{bn_weights_len} BN weights (decay=0.0), "
            f"{decay_weights_len} weights (decay={decay_val_log})"
        )
    return optimizer
